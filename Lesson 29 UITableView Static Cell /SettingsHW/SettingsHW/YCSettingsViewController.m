...
@property (strong, nonatomic) YCSaveAndLoadSettings *saveAndLoadSettings;
...
- (void)viewDidLoad {
    ...
    self.saveAndLoadSettings = [[YCSaveAndLoadSettings alloc] init];
    
    [self.saveAndLoadSettings loadSettings:self.textFieldsAndControlsCollection andTextLabels:self.textLabelCollection];
}
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    ...
    if (textField.tag != self.textFieldCollection.count - 1) {
        [self.textFieldCollection[textField.tag + 1] becomeFirstResponder];
    }
    else {
        [self.textFieldCollection[textField.tag] resignFirstResponder];
    }
...
- (BOOL)textFieldShouldClear:(UITextField *)textField {
    ...
    UILabel *label = self.textLabelCollection[textField.tag];
    label.text = @"";
...
- (BOOL)textField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    switch (textField.tag) {
        ...
        case YCTextFieldTypePhone:
            ...
            break;
        default:
            label = self.textLabelCollection[textField.tag];
            label.text = [textField.text stringByReplacingCharactersInRange:range withString:string];
            break;
    }
    
   [self.saveAndLoadSettings saveSettings:self.textFieldsAndControlsCollection];
...
- (BOOL)phoneTextField:(UITextField *)textField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string {
    ...
    UILabel *label = [self.textLabelCollection objectAtIndex:textField.tag];
...
- (IBAction)actionControls:(id)sender {
    ...
    if ([sender isKindOfClass:[UISlider class]]) {
        self.volumeLabel.text = [NSString stringWithFormat:@"%.0f%%", ((UISlider*)sender).value];
    }
