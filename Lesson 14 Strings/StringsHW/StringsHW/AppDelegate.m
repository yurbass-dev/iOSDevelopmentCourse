...
//  StringsHW
//
//  Created by Yuriy on 23.10.2018.
...
//
//    NSLog(@"First part: \n%@", [text substringToIndex:halfLength]);
//    NSLog(@"Second part: \n%@", [text substringFromIndex:halfLength]);
    
     
     // Another virsion with range
//    NSRange range = NSMakeRange(0, halfLength);
//
//    NSLog(@"\n%@", [text substringWithRange:range]);
//
//    range = NSMakeRange(halfLength, [text length] - halfLength);
//
//    NSLog(@"\n%@", [text substringWithRange:range]);
    
    
    NSArray *arrayWithStrings = [[NSArray alloc] init];
    
    arrayWithStrings = [[text stringByReplacingOccurrencesOfString:@". " withString:@".. "] componentsSeparatedByString:@". "];
...
#pragma mark - Student
    /*
    NSMutableArray *arrayWithWordNSString = [[NSMutableArray alloc] init];
    
    for (NSString *string in arrayWithStrings)
    {
        if ([string containsString:@"NSString"])
            [arrayWithWordNSString addObject:string];
    }
    
    NSLog(@"%@", arrayWithWordNSString);
    */
#pragma mark - Master
    /*
    NSMutableArray *arrayWithReverseWords = [[NSMutableArray alloc] init];
    
    for (NSString *string in arrayWithWordNSString)
    {
        NSArray *array = [string componentsSeparatedByString:@" "];
        NSArray *reversedArray = [[array reverseObjectEnumerator] allObjects];
        
        NSString *stringTemp = [reversedArray componentsJoinedByString:@" "];
        
        stringTemp = [stringTemp stringByReplacingOccurrencesOfString:@". " withString:@" "];       // Delete the dot
        
        [arrayWithReverseWords addObject:stringTemp];
        
        //[arrayWithReverseWords addObject:[[reversedArray componentsJoinedByString:@" "] stringByReplacingOccurrencesOfString:@". " withString:@" "]];
    }
    
    NSLog(@"%@", arrayWithReverseWords);
    */
#pragma mark - Superman
    ...
    while (YES)
    {
        ...
        NSRange range = [text rangeOfCharacterFromSet:charSet options:0 range:searchRange];

        if (range.location != NSNotFound)
        {
            NSRange rangeWord = NSMakeRange(searchRange.location, range.location - searchRange.location);
            
            if (rangeWord.length == 0)
            {
            ...
            NSRange rangeLastChar = NSMakeRange((range.location - 1) - searchRange.location, 1);
            
            NSString *word = [[text substringWithRange:rangeWord] lowercaseString];
            
            NSString *lastChar = [word substringWithRange:(rangeLastChar)];
            
            lastChar = [lastChar uppercaseString];
            
            word = [word stringByReplacingCharactersInRange:rangeLastChar withString:lastChar];
            
            text = [text stringByReplacingCharactersInRange:rangeWord withString:word];
